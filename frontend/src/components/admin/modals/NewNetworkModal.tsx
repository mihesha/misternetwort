...
export const NewNetworkModal = () => {
  ...
  return (
    <>
      {/* NEW NETWORK MANUAL CREATION MODAL */}
      {showNewNetworkModal && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleCreateNewNetwork} className="bg-[#121927] border border-slate-800 rounded-3xl p-6 max-w-md w-full space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="font-black text-white text-base">إضافة شبكة جديدة للمنظومة</h3>
              <button type="button" onClick={() => setShowNewNetworkModal(false)} className="text-slate-400 hover:text-white">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  type="text"
                  required
                  placeholder="مثال: شبكة الريان اللاسلكية"
                  value={newNetName}
              ...
              <div>
                ...
                <input
                  ...
                  value={newNetOwner}
                  onChange={(e) => setNewNetOwner(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-[#1d273a] border border-slate-700 text-white"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  value={newNetPhone}
                  onChange={(e) => setNewNetPhone(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-[#1d273a] border border-slate-700 text-white font-mono"
                />
              </div>

              <div>
                <label className="block text-slate-300 mb-1 font-bold">رقم محفظة جيب المعتمدة:</label>
                <input
                  type="text"
                  placeholder="78XXXXXXX"
                  value={newNetWallet}
                  onChange={(e) => setNewNetWallet(e.target.value)}
                  className="w-full px-3.5 py-2.5 rounded-xl bg-[#1d273a] border border-slate-700 text-white font-mono"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-2">
              ...
              <button
                type="button"
                onClick={() => setShowNewNetworkModal(false)}
